...
try:
    ...
    sys.stderr.write(
        "[thm-badge] curl_cffi is not installed. Run: pip install curl_cffi\n"
    )
    sys.exit(3)

# Chrome/Safari handshakes that get served the JSON instead of the challenge.
# Tried in order; the first 200 wins.
...
def main() -> int:
    ...
    sys.stderr.write(f"[thm-badge] could not fetch profile via browser-TLS; last attempt: {last}\n")
    return 3
